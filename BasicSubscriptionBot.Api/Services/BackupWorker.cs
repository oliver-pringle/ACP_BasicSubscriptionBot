...
// filename-based: KeepDays counts unique YYYY-MM-DD stamps.
//
// The bot DB runs in WAL mode, so a naive `cp <db>` would miss anything still in the
// -wal file — this worker is the only safe backup path. Portfolio convention (lifted
// from ACP_LiquidGuard); this is the BasicSubscriptionBot boilerplate, so every clone
// inherits it. Needs /data/backups on the same bind-mount as the live .db.
public class BackupWorker : BackgroundService
{
    ...
    public BackupWorker(IConfiguration config, Db db, ILogger<BackupWorker> logger)
    {
        ...
        _baseName = config["Backup:BaseName"] ?? "basicsubscriptionbot";
    }
    ...
    public async Task<string?> RunOnceAsync(DateTime nowUtc, CancellationToken ct)
    {
        ...
        // Microsoft.Data.Sqlite keeps the OS file handle open after dispose, and the
        // subsequent File.Move can fail with "file in use".
